...
TagResultsDialog::~TagResultsDialog()
{
    delete ui;
}
...
void TagResultsDialog::on_tableWidget_cellChanged(int row, int column)
{
    ...
    //Set results label
    ui->resLbl->setText(
                QString::number(ui->tableWidget->rowCount()) + ((ui->tableWidget->rowCount() > 1) ?tr(" Results Found") : tr(" Result Found"))
                );
